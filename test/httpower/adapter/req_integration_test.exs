defmodule HTTPower.Adapter.ReqIntegrationTest do
  @moduledoc """
  Integration tests using Req adapter with HTTPower.Test.

  These tests use HTTPower.Test (adapter-agnostic) to prove that the Req adapter
  ...
  setup do
    ...
    # Return adapter options for Req
    {:ok, adapter_opts: [adapter: HTTPower.Adapter.Req]}
  end

  describe "Req adapter: basic HTTP methods" do
  ...
  describe "Req adapter: test mode blocking" do
    test "blocks real requests when test_mode is true" do
      test_mode_blocks_real_requests(HTTPower.Adapter.Req)
    end
  end

  describe "Req adapter: retry logic" do
    test "respects max_retries configuration", %{adapter_opts: adapter_opts} do
      test_retry_respects_max_retries(adapter_opts)
    end
  end

  describe "Req adapter: configured clients" do
